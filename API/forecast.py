...
class forecast:
    ...
    def __init__(self):
        with open('config/datamartconfig.yaml', 'r') as ymlfile:
            cfg = yaml.load(ymlfile, Loader=yaml.SafeLoader)
        self.data=DataAPIs(cfg)

    def Model_train(self, tid, enc_len, dec_len):
        'Train model'
    ...
    def FineTune_Model(self, tid, enc_len, dec_len):
        'Fine tune model'
        model10=ATM_Model(enc_len=enc_len, dec_len=dec_len, epoch=1000, valid_size=199)
    ...
    def Model_predict(self, tid, enc_len, dec_len, serires_input, future):
        'Predict'
        ...
        if (not os.path.exists(model_dir)) or (len(os.listdir(model_dir)) == 0):
            ...
            Logger.instance().info(f'Model for "{tid}" is not existed. Now training..')

            #Tạo và lưu model vào đường dẫn có tid
            self.Model_train(tid, enc_len=enc_len, dec_len=dec_len)
            Logger.instance().info(f'Training finished')
            print('Done')
